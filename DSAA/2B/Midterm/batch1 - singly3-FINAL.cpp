...
public:
  ...
  void update_atNode(Node *n, string s) {
    ...
    curr->data = s;
    print();
    cout << " updated the last node of the term";
  }
  ...
  float evaluate() {
    ...
    while (this->size() != 1) {
      ...
      if (multi) {
        temp = before(multi);
        numTemp = stoi(temp->data);
        numTemp2 = stoi(multi->next->data);
        temp = deleteAtNode(temp);
        temp = deleteAtNode(temp);
        numTemp *= numTemp2;
