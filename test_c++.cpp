...
public:
  ...
  void sortArmAscending() {
    ListNode *i, *j;
    for (i = head; i; i = i->next) {
      for (j = i->next; j; j = j->next) {
        if (isArmstrong(i->data) && isArmstrong(j->data) && i->data > j->data) {
          swap(i->data, j->data);
        }
      }
    }
